...
import {getNewUploader, statusEnum, typeEnum} from './UploaderData'
...
export default function Uploader(props) {
    ...
    useEffect(() => {
        ...
        fileData["uploader"] = getNewUploader(props.subSystem, props.entity, props.type, props.validationExtensionList, props.validationSizeLimit, props.validationItemLimit, [], onStatusChange);
        setFileData({
            ...fileData
        });
    }, []);
    ...
    const onStatusChange = (id) => {
        ...
        if (uploader !== null) {
            ...
            uploader.methods.getUploads().map(obj=> {
                ...
                if (fileObject.status === statusEnum.UPLOAD_SUCCESSFUL) {
                    if (fileObject.id === id) {
                        if (fileObject.statusEnum !== statusEnum.DELETED) {
            ...
            if (uploadComplete) {
                setFileData({
                    ...fileData,
                    "objectSelectedList": fileData["objectSelectedList"],
                    "modalIsOpened": true
                });
            }
        }
        ...
        if (fileData.objectList !== undefined && Object.keys(fileData.objectList).length !== 0) {
            return (
                <React.Fragment>
                    ...
                    <UploaderFileView key={Math.random()}
